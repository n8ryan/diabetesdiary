					...
					<tr>
						...
						<td><span style="color: red" class="glyphicon glyphicon-home"></span>
							<c:out value="${attribs.address}" /> <c:out
								value="${attribs.city}" /> <c:out value="${attribs.state}" />
							<c:out value="${attribs.zip}" /></td>
					</tr>
					<tr>
						<td>Web site</td>
						<td><c:choose>
							...
							<form class="col-md-10" action="changeSubscriptionType"
								...
								<div class="input-group add-on">
									<input type="hidden" name="id" value="${attribs.id}" /> <select
										...
										<option value="Anual Subscription">Annual
					...
					<tr>
						...
						<td>
							<c:if test="${empty attribs.selectClub}">
								<form class="col-md-11" action="addAddress" method="post">
									<div class="input-group add-on">
										<input type="hidden" name="id" value="${attribs.id}" /> <select
											name="selectClub" class="form-control input">
					...
					<c:otherwise>
						<div class="col-md-6-">
							<form class="col-md-10" action="writeBio" method="post">
								<input type="hidden" name="id" value="${attribs.id}" /> <label
									for="background" id='background'>Biography</label>
								<textarea class="form-control" id="background" name="background"
									rows="5" placeholder="Write a short Bio"></textarea>
								<br>
